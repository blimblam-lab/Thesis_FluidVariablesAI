...
#Rel RMSE 1: 0.036 | 2: 0.0 | 3: 0.0


path = '/LSTM Info/LSTMTests/f&dsep/Shifts/NewTries/'
plt.close('all')
...
model.save('LSTMd80tsBatch1', save_format='h5')
hist = pd.DataFrame(runfit.history)    #progress printen
hist.to_csv('LSTMd80tsBatch1hist.csv')
'''
model = K.models.load_model('LSTMd80tsBatch1')
...
ModelPerfo.index.name = 'LSTM_d_80tsBatch1_perfo'
ModelPerfo.to_csv('LSTM_d_80tsBatch1_perfo.csv', header=['Value'])
ModelPerfo.to_latex('LSTM_d_80tsBatch1_perfo.tex', header=['Value'])
'''
#plotting
hist = pd.read_csv('LSTMd80tsBatch1hist.csv')
